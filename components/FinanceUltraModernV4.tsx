/**
 * 💰 FINANCE ULTRA MODERN V4 - STANDARDISÉ SUR PROJECTS
...
const FinanceUltraModernV4: React.FC = () => {
  ...
  const loadData = async () => {
    try {
      ...
      const [invoicesData, expensesData, budgetsData, usersData] = await Promise.all([
        financeService.getInvoices(), // Récupérer toutes les factures pour l'instant
        financeService.getExpenses(), // Récupérer toutes les dépenses pour l'instant
        financeService.getBudgets(), // Récupérer tous les budgets pour l'instant
        userService.getAll()
      ]);
  ...
  const filteredData = useMemo(() => {
    ...
    return data.filter(item => {
      const matchesSearch = !filters.search || 
        (item.client || item.description || item.name || '').toLowerCase().includes(filters.search.toLowerCase()) ||
        (item.number || '').toLowerCase().includes(filters.search.toLowerCase());
      
      const matchesStatus = !filters.status || item.status === filters.status;
      const matchesCategory = !filters.category || item.category === filters.category;
      
      const matchesAmount = item.amount >= filters.amountRange.min && item.amount <= filters.amountRange.max;
      
      const matchesDate = !filters.dateRange.start || !filters.dateRange.end || 
        (item.dueDate && new Date(item.dueDate) >= new Date(filters.dateRange.start) && 
         new Date(item.dueDate) <= new Date(filters.dateRange.end));
      
      return matchesSearch && matchesStatus && matchesCategory && matchesAmount && matchesDate;
    });
  }, [getCurrentData, filters]);
  ...
  const sortedData = useMemo(() => {
    return [...filteredData].sort((a, b) => {
      ...
      if (sortBy === 'dueDate' || sortBy === 'createdAt' || sortBy === 'issueDate') {
          ...
          <div className="flex justify-between items-center py-6">
            ...
            <div className="flex space-x-3">
              ...
              >
                ...
                {activeTab === 'invoices' ? 'Nouvelle Facture' : 
                 activeTab === 'expenses' ? 'Nouvelle Dépense' : 'Nouveau Budget'}
              </button>
              <button
          ...
          <div className="bg-white rounded-lg shadow p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-semibold text-gray-900">{metrics.totalInvoices}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg shadow p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-semibold text-gray-900">{metrics.paidInvoices}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg shadow p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-semibold text-gray-900">{metrics.pendingInvoices}</p>
              </div>
            </div>
          </div>
        ...
        <div className="bg-white rounded-lg shadow mb-6">
          <div className="border-b border-gray-200">
            <nav className="-mb-px flex space-x-8 px-6">
              {[
                { id: 'invoices', name: 'Factures', count: invoices.length },
                { id: 'expenses', name: 'Dépenses', count: expenses.length },
                { id: 'budgets', name: 'Budgets', count: budgets.length }
              ].map((tab) => (
                <button
                  key={tab.id}
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between space-y-4 lg:space-y-0">
            ...
            <div className="flex flex-wrap items-center space-x-4">
              ...
              >
                ...
                <option value="dueDate-asc">Échéance proche</option>
                <option value="dueDate-desc">Échéance lointaine</option>
              </select>
            </div>
        ...
        {sortedData.length === 0 ? (
          <div className="text-center py-12">
            ...
            <div className="mt-6">
              ...
              >
                <PlusIcon className="h-4 w-4 mr-2" />
                {activeTab === 'invoices' ? 'Créer une facture' : 
                 activeTab === 'expenses' ? 'Créer une dépense' : 'Créer un budget'}
              </button>
            </div>
          </div>
        ) : (
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <h3 className="text-lg font-medium text-gray-900 mb-2">
                          {item.client || item.description || item.name || 'Sans titre'}
                        </h3>
                    ...
                    <thead className="bg-gray-50">
                      <tr>
                        <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                          {activeTab === 'invoices' ? 'Facture' : activeTab === 'expenses' ? 'Dépense' : 'Budget'}
                        </th>
                        <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                          Client/Description
                        </th>
...
export default FinanceUltraModernV4;
