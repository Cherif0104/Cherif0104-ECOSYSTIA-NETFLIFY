...
async function createUserDocument(user, i) {
  try {
    console.log(`\n[${i+1}/10] Création document: ${user.name} (${user.role})`);
    ...
    console.log(`✅ Document créé: ${user.name} (${docRes.$id})`);
    return true;
    
  } catch (error) {
    if (error.code === 409) {
      console.log(`⚠️ Document existe déjà: ${user.name}`);
      return true;
    } else {
...
async function main() {
  ...
  for (let i = 0; i < existingUsers.length; i++) {
    const result = await createUserDocument(existingUsers[i], i);
    if (result) success++;
    else errors++;
  ...
  console.log(`\n🎉 TERMINÉ!`);
  console.log(`✅ ${success} documents créés | ❌ ${errors} erreurs`);
