...
// Configuration Appwrite (utiliser les mêmes IDs que dans appwriteService.ts)
const client = new Client()
  .setEndpoint('https://nyc.cloud.appwrite.io/v1')
  .setProject('68ee2dc2001f0f499c02'); // ID correct du projet

const databases = new Databases(client);
const DATABASE_ID = '68ee527d002813e4e0ca'; // ID correct de la base de données

console.log('🔄 Configuration des collections Appwrite...');
console.log('Endpoint:', client.config.endpoint);
console.log('Project ID:', client.config.project);
console.log('Database ID:', DATABASE_ID);

// Collections essentielles pour la production
const collections = [
  {
    collectionId: 'users',
    name: 'Utilisateurs',
...
async function setupProductionCollections() {
  ...
  try {
    ...
    console.log(`📋 ${response.collections.length} collections existantes trouvées\n`);
